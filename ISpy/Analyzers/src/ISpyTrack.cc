...
ISpyTrack::analyze( const edm::Event& event, const edm::EventSetup& eventSetup)
{
  ...
  {	    	
         ...
         track != trackEnd; ++track)
    {
      ...
      //IgCollectionItem item2 = trackerMuonCollection.create();
      ...
      /*
      item2[T_PT]  = (*track).pt();
      item2[T_PHI] = (*track).phi();
      item2[T_ETA] = (*track).eta();
      item2[T_CHARGE] = (*track).charge();
      item2[T_CALO_E] = 0;
      item2[T_RP] = IgV3d((*track).vx()/100.0,
                          (*track).vy()/100.0,
                          (*track).vz()/100.0);
      */
      ...
      std::cout << (*track).innerOk() << " " << (*track).innerMomentum().x() << " " <<  (*track).innerMomentum().y() << "  " << (*track).innerMomentum().z() << std::endl;
