...
AlignmentQC8::AlignmentQC8(const edm::ParameterSet& cfg): GEMBaseValidation(cfg)
{
	...
	for(int i = 0; i<SC; i++)
	{
		for(int j=0;j<maxNeta;j++)
		{
			...
			hname = Form("hColEtaPx/Pz_%d_%d",i/5+1,j+1);
			chTitle = Form("Angular distribution (Px/Pz) Column %d iEta%d",i/5+1,j+1);
			hColEtaPxPz[(int)i/5+1][j] = fs->make<TH1D>(hname,chTitle,500,-0.05,0.05);
		}
	}
}

void AlignmentQC8::bookHistograms(DQMStore::IBooker & ibooker, edm::Run const & Run, edm::EventSetup const & iSetup ) {
	//useless here but necessary to get the module compiled
}
		...
		if ( (tlp2.x()>(min_x)) && (tlp2.x() < (max_x)) )
		{
			...
			if(tmpRecHit)
			{
				...
				if(fabs(atan(trajPy/trajPz))<0.1)
				{
					...
					if(fabs(atan(trajPy/trajPz))<0.01) // tracks must pass in the same eta partition for all the chambers in the column
					{
						hColEtaPxPz[(int)idx/10][imRoll]->Fill(trajPy/trajPz);
					}
				}
			}
		}
